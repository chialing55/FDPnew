...
plot_all <- function() {
  par(family = axis_text_family, mar = c(3, 6, 2, 4), xpd = NA)
  ...
  x_max <- if (is.finite(max_value) && max_value > 0) max(pretty(c(0, max_value))) else 1
...
grDevices::pdf(pdf_path, width = 5.9, height = 12, onefile = TRUE, family = "sans")
draw_output()
dev.off()

png(png_path, width = 1770, height = 3600, res = 300, type = "cairo")
draw_output()
dev.off()
