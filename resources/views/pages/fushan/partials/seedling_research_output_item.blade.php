  ...
  @else
    ...
    @foreach ($compositionSummary['surveys'] as $survey)
      @php
        ...
        $topNewText = collect($topNewFive)->map(fn ($row) => $row['csp'] . ' (' . number_format($row['count']) . ' 株)')->implode('、');
        ...
        if ($survey['sequence'] === 1) {
          ...
          $surveySentence .= '，平均密度為' . $decimal($survey['density']) . '棵/m2。此次調查中新增小苗數量最多的 5 個樹種分別是' . ($topNewText !== '' ? $topNewText : '無新增小苗');
          if ($survey['new_total'] > 0 && isset($topNewFive[0])) {
            $surveySentence .= '，' . $topNewFive[0]['csp'] . '佔整體新增株數的' . $percent($survey['new_top_percent']) . '%';
          }
          $surveySentence .= '。死亡小苗中株數最高的前 5 個樹種為' . ($topDeadText !== '' ? $topDeadText : '無死亡小苗') . '。';
        }
      @endphp
...
@elseif ($item === 'survival-growth')
  ...
  @else
    ...
    @endif
    <p style='margin:0 0 10px;'>
      我們篩選於前一次小苗調查中存活植株數量 ≥ 10 株的樹種，分別計算各樹種之年存活率與年生長率。
    </p>
    <p style='margin:0 0 10px;'>
    ...
    @foreach ($intervals as $interval)
      ...
      <p style='margin:0 0 6px;'>
        挑選{{ $interval['previous_month_text'] }}之調查中小苗數量 ≥ 10 株的樹種進行計算，N 為{{ $interval['previous_month_text'] }}所調查到之小苗數量，S 為{{ $interval['current_month_text'] }}存活之舊苗數量。
      </p>
