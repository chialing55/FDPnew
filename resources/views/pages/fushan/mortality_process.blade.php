...
@section('rightbox')
    <div class='text_box' style="width:min(900px, 92vw); text-align:left;">
        ...
        @if (session('process_summary'))
            ...
            <div
                style="margin:14px 0; padding:12px; border:1px solid rgba(0,0,0,.08); background:rgba(255,255,255,.72); border-radius:6px;">
                <div style="font-weight:700; margin-bottom:8px;">處理結果</div>
                <div>`map = '='` 更新筆數：{{ $summary['updated_map_count'] ?? 0 }}
            </div>
    ...
    <div style="margin-top:18px; display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
        <form method="POST" action="{{ route('admin.fushan.mortality.process.basic') }}" style="margin:0;">
            @csrf
            <button type="submit" {{ !empty($basicProcessed) ? 'disabled' : '' }}
    ...
    <div style="margin-top:18px; display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
        <form method="POST" action="{{ route('admin.fushan.mortality.process.tree-individuals') }}" style="margin:0;">
            @csrf
            <button type="submit" {{ empty($basicProcessed) || empty($hasImportStageStemids) ? 'disabled' : '' }}
    ...
    <div style="margin-top:18px; display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
        <form id="people-process-form" method="POST" action="{{ route('admin.fushan.mortality.process.people') }}"
            style="margin:0;">
            @csrf
            <input type="hidden" name="census" value="{{ $derivedProcessCensus ?? '' }}">
            <button type="submit" {{ !empty($peopleProcessed) || empty($derivedProcessCensus) ? 'disabled' : '' }}
    ...
    <div style="margin-top:18px; display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
        <form method="POST" action="{{ route('admin.fushan.mortality.process.comments') }}" style="margin:0;">
            @csrf
            <button type="submit" {{ empty($commentsRemaining) ? 'disabled' : '' }}
    ...
    <div style="margin-top:18px; display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
        <form id="census-record-import-form" method="POST"
            action="{{ route('admin.fushan.mortality.process.census-records') }}" style="margin:0;">
            @csrf
            <input type="hidden" name="census" value="{{ $derivedProcessCensus ?? '' }}">
        ...
        @if (empty($derivedProcessCensus))
            <span id="census-record-import-hint" style="color:#6b7280;">目前無法由 `import_stage.date` 自動判斷對應
                census，按鈕已停用。</span>
