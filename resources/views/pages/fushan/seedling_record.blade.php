...
<body>
    <header>
        <div class='iflex'>{{ $title }}<span
                style='margin-left:250px; font-size: 12px;'>調查者__________________________________________紀錄者____________________</span>
        </div>

        <div style='text-align: right; margin-top: -25px; margin-right:20px; font-size: 12px;'><span
                class="pagenum"></span><span class='page'>
    ...
    <footer style="margin-bottom: 10px;">
        <p>[樣區上方光度] U：多層樹冠；I：一層樹冠；G：孔隙 <span style='padding-left: 30px;'>[長度/葉片數]
                -1：因故沒有測量；-2：DBH>=1；-4：死亡；-6：消失；-7：枝幹死亡但個體存活</span><span style='padding-left: 30px;'>[狀態]
                A：存活；G：見環不見苗；D：死亡；N：消失</span></p>
    </footer>
                ...
                @if (isset($record[$plot[$k]]))
                    @for ($i = 0; $i < count($record[$plot[$k]]); $i++)
                        <tr>
                            ...
                            <td class="">
                                @if ($record[$plot[$k]][$i]['y'] != '-1')
                                    {{ $record[$plot[$k]][$i]['y'] }}
                                @endif
                            </td>
                            @if (strlen($record[$plot[$k]][$i]['note']) > 35)
                                <td class="talble-right" colspan="9" style='font-size: 10px'>
                                    {{ $record[$plot[$k]][$i]['note'] }}</td>
                            @else
                                <td class="talble-right" style='font-size: 10px'>{{ $record[$plot[$k]][$i]['note'] }}
                                </td>
                                <td class=""></td>
                                <td class=""></td>
                                <td class=""></td>
