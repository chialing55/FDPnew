...
<div>
    ...
    @if ($entry == '')
        @if ($census == $census2)
            <div class='text_box'>
                <h6>新建種子雨收集日期<span style='font-weight: 500; color: red; margin-left: 20px ;'>{{ $submitformnote }}</span>
                </h6>

                <form wire:submit.prevent="submitForm" method="POST">
                    <p>census：{{ $census }}</p>
                    <p>調查日期：<input name='date' id='date' type='date' placeholder="YYYY-MM-DD"
                            style='width:120px; ' class='fs100' wire:model="date"></p>
                    <p>調查人員：<select name='person1' id='person1' class="" style='width:85px;'
                            wire:model="person1">
                            @for ($i = 0; $i < count($personlist); $i++)
                                <option value="{{ $personlist[$i] }}">{{ $personlist[$i] }}</option>
                            @endfor
                        </select>, <select name='person2' id='person2' class="" style='width:85px;'
                            wire:model="person2">
                            @for ($i = 0; $i < count($personlist); $i++)
                                <option value="{{ $personlist[$i] }}">{{ $personlist[$i] }}</option>
                            @endfor
                        </select>, <input name='person3' id='person3' type=text style='width:150px'
                            wire:model="person3" />
                        (請以半形「,」分隔，並留一空格)
                    </p>
                    <p>note：<input name='note' id='note' type='text' style='width:220px; ' class='fs100'
                            wire:model="note"></p>
        ...
        @endif
    @else
        <div class='text_box keepenter'> <b>第 {{ $thiscensus }} 次 ({{ $census2date['date'] }}) 調查資料未完成</b> <button
                type='submit' wire:click='direntry({{ $thiscensus }})' style='margin-left: 20px;'>繼續輸入</button>
        </div>
    @endif
    ...
    <div class='text_box dateinfo'>
        ...
        <form wire:submit.prevent="submitForm3" method="POST">
            <p>census：<input name='census3' id='census3' type='text' style='width:120px; ' class='fs100'
                    wire:model="census3"></p>
            <p>調查日期：<input name='date3' id='date3' type='date' placeholder="YYYY-MM-DD" style='width:120px; '
                    class='fs100' wire:model="date3"></p>
            <p>調查人員：<select name='person31' id='person31' class="" style='width:85px;' wire:model="person31">
                    @for ($i = 0; $i < count($personlist); $i++)
                        <option value="{{ $personlist[$i] }}">{{ $personlist[$i] }}</option>
                    @endfor
                </select>, <select name='person32' id='person32' class="" style='width:85px;'
                    wire:model="person32">
                    @for ($i = 0; $i < count($personlist); $i++)
                        <option value="{{ $personlist[$i] }}">{{ $personlist[$i] }}</option>
                    @endfor
            ...
            <p>note：<input name='note3' id='note3' type='text' style='width:220px; ' class='fs100'
                    wire:model="note3"></p>
            <button type='submit'>輸入</button>
        </form>
    </div>
