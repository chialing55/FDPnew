...
$(".listlink").on("click", function () {
    ...
    if (typeof type != "undefined") {
        location.href = `/admin/fushan/tree/${type}`;
    }
});
...
document.addEventListener("livewire:init", () => {
    if (window.__boundDataEvent) return;
    window.__boundDataEvent = true;
    Livewire.on("data", ({ record, emptytable, csplist }) => {
        data = record;
...
function cellfunction(tableType, container, row, col, prop) {
    ...
    if (tableType == "data") {
        ...
        if (
            (container.handsontable("getData")[row][5] != "0") &
            (container.handsontable("getData")[row][7] != "y")
        ) {
            cellProperties.readOnly = true;
            if (col == 5 || col == 4) {
                cellProperties.readOnly = false;
...
function fstreetable(data, thispage, pps) {
    ...
    var colWidths = [
        120, 25, 25, 25, 25, 80, 40, 120, 50, 50, 60, 50, 160, 50, 160,
    ];
    var colHeaders = [
        "Date",
        "20x",
...
document.addEventListener("livewire:init", () => {
    // 避免 Livewire 重繪造成重複綁定（很常見）
    if (window.__boundProgressEvents) return;
    window.__boundProgressEvents = true;

    Livewire.on("reProgress", ({ plots }) => {
    ...
    Livewire.on("rePlotsentry", ({ sqx, sqy }) => {
...
function canselectchcek(plots) {
    $(".plottable")
        .attr("show", "0")
        .removeClass("plotselect")
        .removeClass("cannotselect")
        .addClass("canselect");
...
document.addEventListener('livewire:init', () => {
  if (window.__boundStemEvents) return;
  window.__boundStemEvents = true;
...
function fstreeupdatatable(stemid, stemdata, csplist, from) {
    ...
    var handsontable1 = createHandsontable(
        container1,
        columns1,
        stemdata[0],
        saveButtonName,
    ...
    var handsontable2 = createHandsontable(
        container2,
        columns2,
        stemdata2,
        saveButtonName,
    ...
    container2
        .parent()
        .find(`button[name=datasave${stemid}]`)
        .click(function () {
            $(".datasavenote").html("");
...
document.addEventListener('livewire:init', () => {
    if (window.__boundUpdataEvent) return;
    window.__boundUpdataEvent = true;

    Livewire.on('updata', ({ emptytable, csplist, updatelist }) => {
        emptytable_addtree = emptytable;
        csplist = csplist;
...
document.addEventListener('livewire:init', () => {
    if (window.__boundInitTablesorter) return;
    window.__boundInitTablesorter = true;

    Livewire.on('initTablesorter', ({ tablePlot, data, tag, mapfile }) => {
        console.log(data);
